...
def main():
    ...
    outpath = Path("/Users/terooatt/Downloads/scJDO/scratchpad_run/saddle_readouts.json")
    outpath.write_text(json.dumps(results, indent=2))
    print(f"\nSaved: {outpath}")

    # Also save the curves themselves for downstream plotting.
    np.savez_compressed(
        "/Users/terooatt/Downloads/scJDO/scratchpad_run/saddle_readouts_curves.npz",
        grid=grid,
