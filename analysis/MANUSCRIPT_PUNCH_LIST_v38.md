...
| 7.4 | **"pre-correction top-9 was dominated by Cdc20 / Ccnb1 / Cenpf"** | ✗ **NOT REPRODUCIBLE, checked end-to-end from raw data.** Ran the full pipeline (`analysis/supp_multiome_note/run_uncorrected_multiome_pipeline.py`) starting from the raw 10x E18 matrix at `/Users/terooatt/Documents/…/multiomic/filtered_feature_bc_matrix/` with `DO_CC_REGRESSION = False` at every stage. Result: <br>• **CC signal IS present in the uncorrected joint FA latent** (\|r\| with G2M: factor 1 = 0.50, factor 2 = 0.32, factor 0/3/4 all < 0.10). So the mechanistic premise — "CC signal is encoded in the manifold geometry of the learned drift field" — is correct. <br>• **But the ExcNeuron top-9 is still not mitotic-dominated.** Uncorrected top-9 on the truly-raw pipeline: **Cacna2d3, Col19a1, Neto1, Grik1, Npas1, Reln, Galntl6, Thrb, Kctd8** — neuronal differentiation markers, zero Cdc20 / Ccnb1 / Cenpf / Mki67 / Top2a / Aurkb / etc. hits. <br>Full comparison: `analysis/results/supp_multiome/uncorrected_pipeline/top9_comparison.json`. Three pipelines checked — shipped uncorrected, shipped CC-regressed, and this truly-uncorrected re-run — all three return neurogenic top-9 lists with **zero mitotic-gene hits**. | **Rewrite the Supplementary Note's Cdc20/Ccnb1/Cenpf sentence.** Two evidenced alternatives: (a) keep the general mechanism ("the uncorrected joint FA latent carries substantial CC signal — for this dataset, \|r\| = 0.50 with G2M on factor 1") and describe what actually appears in the top-9 (three neurogenic gene lists across the three pipeline variants); (b) if the mitotic-dominance behavior was observed on a different dataset than E18 mouse brain, name that dataset here rather than E18. The current specific claim about E18 top-9 mitotic dominance is contradicted by three independent runs. |
...
## Second-pass resolutions (from author confirmation + additional runs)

- **Fig 5 cell count 24,999 vs 27,538:** author confirmed **24,999** is authoritative — the notebook's stratified subsample.
- **Fig 3 Palantir-vs-DPT ρ = 0.08:** 9 additional configurations tested, all ρ ∈ [0.84, 0.99]; recommendation is now full retraction of the number rather than a request for the config.
- **Concordance-04-compare notebook:** was `concordance_splicejac_dynamo.{ipynb,pdf,png}` — renamed to `concordance_04_compare.*` so the four-notebook chain reads in numeric order and matches how the three per-method notebooks describe it.
- **Figure6_multiome_branch_specificity_validation.ipynb:** regenerated from `build_branch_specificity_notebook.py`; now committed alongside the outputs it produces.
- **`build_cc_regressed_notebook.py` filename drift:** builder now writes `Figure6_01_multiome_drift_cellcycle_regressed_FA.ipynb`, the name used by the committed notebook and by manuscript v38's Code Availability.
- **Multiome pre-correction Cdc20 claim:** ✗ definitively not reproducible. Truly-uncorrected pipeline (raw 10x → no CC regression on RNA or ATAC → joint FA → drift → instability) run end-to-end. Uncorrected latent DOES carry CC signal (factor 1 \|r\| = 0.50 with G2M), so the mechanistic premise is correct, but the ExcNeuron top-9 comes out as neuronal differentiation markers (Cacna2d3, Col19a1, Neto1, Grik1, Npas1, Reln, Galntl6, Thrb, Kctd8) — zero mitotic hits. See item 7.4 above.
