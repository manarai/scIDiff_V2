...
def load_scp295_met():
    ...
    import os
    import scipy.sparse
    p = Path(os.environ.get("SCJDO_SCP295_H5AD", "") or
             "/Users/terooatt/Documents/Project_scQDiff/02_scQDiff/scIDIFF_anndata/data/SCP295/scp295.h5ad")
    if not p.exists():
        raise FileNotFoundError(
            f"SCP295 h5ad not found at {p}. Set SCJDO_SCP295_H5AD env var or "
            f"see final_manuscript/DATA.md §3 for download instructions."
        )
    a = sc.read_h5ad(p)
    sc.pp.normalize_total(a, target_sum=1e4)
    sc.pp.log1p(a)
